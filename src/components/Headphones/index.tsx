...
function Header({ onSelect, onToggleMenu, showMenu }: {
      ...
      <nav className="flex justify-between items-center px-6 py-4 border-b border-gray-700 sticky top-0 z-40 bg-black">
        ...
        <div className="cursor-pointer">
          <svg width="24" height="20" xmlns="http://www.w3.org/2000/svg">
            <path d="M0 1h4l2.68 13.39..." fill="none" stroke="#fff" strokeWidth="2" />
          </svg>
        </div>
      </nav>
      {showMenu && (
...
function CategoryCard({ type, product, onClick }: {
  type: ProductType;
  product: Product;
  onClick: () => void;
}) {
...
export default function Page() {
  ...
  return (
    <div className="bg-black text-white min-h-screen font-sans overflow-x-hidden relative">
      <Header
        onSelect={(type) => { setShowModal(type); setShowMenu(false); }}
      ...
      <Hero onClick={() => setShowAllProducts(true)} />
